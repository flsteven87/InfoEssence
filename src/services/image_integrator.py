...
class ImageIntegrator:
    ...
    def process_title(self):
        ...
        # 計算 title 的寬度 
        self.title_width = self.img.width - self.title_left_margin - self.title_right_margin
        ...
        if split_index != -1:
            ...
            if len(first_part) <= 17 and len(second_part) <= 17:
            ...
            # 用第二個標點符號切分，檢查前後兩段是否都小於 17 個字，若都小於則將其切分並放置到第一跟第二行，若只有一段小於則17個字第一行，剩餘第二行
                ...
                if len(first_part) <= 17 and len(second_part) <= 17:
                    ...
                    self.first_line = self.ig_title[:17]
                    self.second_line = self.ig_title[17:]
            else:
                self.first_line = self.ig_title[:17]
                self.second_line = self.ig_title[17:]
        else:
